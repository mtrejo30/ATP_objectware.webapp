...
public class ApplicationAuthenticationProvider extends JdbcDaoSupport implements UserDetailsService
{
	 ...
	 */
	public static final String APPLICATION_QUERY = "select username, password, roles, status from user where username = ?";
	...
	protected List<UserDetails> loadUsersByUsername(String username)
	{
		return getJdbcTemplate().query(APPLICATION_QUERY, new String[] { username }, new RowMapper<UserDetails>()
		{
			public UserDetails mapRow(ResultSet rs, int rowNum) throws SQLException
			{
				...
				return new User(username, password, enabled, accountNonExpired, true, true, getAuthorityListResolver().getAuthorityListFrom(rs));
			}
		});
	}
